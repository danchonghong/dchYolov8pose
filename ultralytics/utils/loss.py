...
from ultralytics.utils.metrics import OKS_SIGMA, WiseIouLoss, wasserstein_loss, bbox_inner_iou, bbox_mpdiou, \
    bbox_inner_mpdiou, bbox_focaler_iou, bbox_focaler_mpdiou
...
class v8PoseLoss(v8DetectionLoss):
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        super().__init__(model)
        self.kpt_shape = model.model[-1].kpt_shape  # 17 3
        self.bce_pose = nn.BCEWithLogitsLoss()
        is_pose = self.kpt_shape == [17, 3]
        nkpt = self.kpt_shape[0]  # number of keypoints 17
    ...
    def kpts_decode(anchor_points, pred_kpts):
        ...
        y = pred_kpts.clone()
        y[..., :2] *= 2.0  # 坐标乘2
        y[..., 0] += anchor_points[:, [0]] - 0.5  # x+anchor_x-0.5
        y[..., 1] += anchor_points[:, [1]] - 0.5  # y+anchor_y-0.5
        return y

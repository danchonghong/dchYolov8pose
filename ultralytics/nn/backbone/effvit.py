...
class EfficientViT(torch.nn.Module):
    ...
    def forward(self, x):  # 1 3 640 640 1 3 256 256 1 3 32 32
        outs = []
        x = self.patch_embed(x)  # 1 64 80 80 1 64 32 32
        x = self.blocks1(x)  # 1 64 80 80 不改变大小和形状
        outs.append(x)
        x = self.blocks2(x)  # 1 128 40 40
        outs.append(x)
        x = self.blocks3(x)  # 1 192 20 20 1  64  8  8
        outs.append(x)
        return outs
...
#     print(len(EfficientViT_M0().forward(torch.Tensor(1, 3, 640, 640))))
...
class ActivationsAndGradients:
    ...
    def save_gradient(self, module, input, output):
        if not hasattr(output, "requires_grad") or not output.requires_grad:
            ...
            return "You can only register hooks on tensor requires grad."
...
class yolov8_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        for l in layer:
            if l == 15:
                for j in (1, 2, 3):
                    target_layers.append(model.model[l].cv4[2][j])
            else:
                target_layers.append(model.model[l])
        # target_layers = [model.model[l] for l in layer]
        # print(target_layers.__len__())
...
def get_params():
    params = {
        'weight': 'runs/pose/train74/weights/best.pt',  # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
        'method': 'GradCAM',
        # 'method': 'GradCAM',
        # GradCAMPlusPlus, GradCAM, XGradCAM, EigenCAM, HiResCAM, LayerCAM, RandomCAM, EigenGradCAM
        'layer': [5, 8, 10, 11, 13, 14],
        # 'layer': [15],
...
if __name__ == '__main__':
    ...
    model(r'all/ztest/0411_17_1638.png', 'result')
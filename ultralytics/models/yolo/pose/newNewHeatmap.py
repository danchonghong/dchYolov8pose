...
class yolov8_heatmap:
    ...
    def __call__(self, img_path, save_path):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            cam_image = self.draw_detections(det_box_restored, self.colors[0],
                                             f'{self.model_names[0]} {float(post_result[i].max()):.2f}',
                                             cam_image)
...
def get_params():
    params = {
        'weight': './runs/pose/train74/weights/best.pt',
        'cfg': r'F:\ultralytics-main\ultralytics-main\ultralytics\cfg\models\v8\yolov8s-star.yaml', # 这里选择与训练上面模型权重相对应的.yaml文件路径
        'device': 'cuda:0',
        'method': 'GradCAM',  # GradCAMPlusPlus, GradCAM, XGradCAM
        'layer': f'model.model[5]',
...
if __name__ == '__main__':
    ...
    model(r'all/ztest/0411_17_1638.png', f'result/5')
